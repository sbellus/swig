 ...
 * ------------------------------------------------------------------------- */
/* Keywords plus the declaration modifiers (final, override, static, ...): the Swift 6.1+
 * parser reads e.g. "public class override {" as a modifier list, so those names must be
 * backticked too. */
static bool isSwiftKeyword(const char *name) {
  static const char *keywords[] = {
    ...
    "throws",          "true",        "try",    "convenience", "dynamic",   "final",   "fileprivate", "infix",    "internal", "lazy",     "mutating",
    "nonisolated",     "nonmutating", "open",   "optional",    "override",  "package", "postfix",     "prefix",   "private",  "public",   "required",
    "static",          "unowned",     "weak",   NULL};
...
class SWIFT : public Language {
  ...
  List *director_open_stub_keys;       /* one "name/arity" key per buffered stub */
  List *director_open_stub_texts;      /* buffered open func stubs, same order */
  Hash *director_open_emitted;         /* key -> number of real open funcs emitted */
   ...
   * the Swift signature names the class.  Only a type whose base really is the enclosing
   * class qualifies: any other opaque type (int*, void*, ...) is left for the SWIGTYPE_p_*
   * wrapper, so a member and the director dispatch calling it agree on the parameter type.
   * Consumes and replaces the passed string. */
  String *selfClassNameIfOpaque(String *cn, SwigType *t) {
    ...
    Replaceall(base, "class ", "");
    Replaceall(base, "struct ", "");
    String *cname = Getattr(cls, "name");
    String *ctype = Getattr(cls, "classtypeobj");
    bool is_self = (cname && Equal(base, cname)) || (ctype && Equal(base, ctype)) || Equal(base, proxy_class_name);
    Delete(base);
    Delete(stripped);
    Delete(resolved);
    if (!is_self)
  ...
  /* Swift argument label of the i-th (0-based) wrapped parameter: the C++ parameter
   * name, else SWIG's local name, else "argN", keyword-escaped and made unique within
   * the signature.  The proxy method declaration and the director dispatch call into
   * it must both come from here so an open func and its caller agree. */
  String *swiftParamLabel(Parm *p, int i, Hash *seen) {
    ...
    if (pname && Len(pname) > 0)
      ...
      label = NewStringf("arg%d", i + 1);
    label = swiftEscapeIdentifier(label);
    if (Getattr(seen, label)) {
      String *unique = NewStringf("%s_%d", label, i + 1);
      Delete(label);
      label = unique;
    }
    Setattr(seen, label, "1");
    return label;
  }
...
public:
  SWIFT() :
    ...
    director_open_stub_keys(NULL),
    director_open_stub_texts(NULL),
    director_open_emitted(NULL),
  ...
  virtual int functionWrapper(Node *n) {
    ...
    /* A director class method reaches C++ two ways: called on the Swift object the
     * director was created from (the base implementation is meant), or on a proxy
     * wrapping that C++ object (virtual dispatch is meant, so an override in the
     * Swift subclass runs).  Only Swift can tell those apart - its proxy knows
     * whether it is the director's own object - so it passes the decision in.
     * Python and Java get this for free: their directors derive from Swig::Director,
     * so the wrapper compares the director's stored self with the calling object. */
    if (isDirectorVirtual(n, static_flag)) {
      if (gencomma) {
        Printf(f->def, ", ");
        Printf(c_param_decl, ", ");
      }
      Printf(f->def, "int swig_upcall");
      Printf(c_param_decl, "int");
    }
     ...
     *
     * A director class method instead takes the decision as the swig_upcall argument
     * (see the C function header above): the non-virtual branch is what the director's
     * own Swift object needs - calling the method virtually from there would dispatch
     * straight back into it - and the virtual branch is what a proxy wrapping the C++
     * object needs, so an override in the Swift subclass runs.
     *
     * For non-director classes the upcall branch is dead code — keep if(false) so
     * the compiler elides it cleanly. */
    Replaceall(actioncode, "if (upcall)", isDirectorVirtual(n, static_flag) ? "if (swig_upcall)" : "if (false)");
  ...
  void emitSwiftProxyMethod(Node *n, String *dst_code, bool is_static) {
    ...
    for (i = i_start, p = p_start; i < num_args; i++) {
      ...
      String *swift_arg_name = swiftParamLabel(p, i, seen_arg_names);
    ...
    /* See functionWrapper: a director class method takes the upcall decision as a
     * trailing argument.  swigDirectorSelf is true only on the Swift object the
     * director was created from, which is exactly when the base implementation
     * (or, for a pure virtual, DirectorPureVirtualException) is wanted. */
    bool is_director_virtual = isDirectorVirtual(n, is_static);
    if (is_director_virtual) {
      if (Len(c_args) > 0)
        Printf(c_args, ", ");
      Printf(c_args, "swigDirectorSelf ? 1 : 0");
    }
    ...
    /* A virtual method of a director class is the member a Swift subclass overrides,
     * so it must be "open" rather than "public". */
    const char *access = is_director_virtual ? "open" : "public";
  ...
  /* A non-static virtual member of the director class currently being wrapped.  Its
   * C wrapper takes the extra swig_upcall argument and its Swift proxy method is the
   * overridable "open func". */
  bool isDirectorVirtual(Node *n, bool is_static) {
    String *storage = Getattr(n, "storage");
    String *nodetype = Getattr(n, "nodeType");
    return director_flag && !is_static && is_wrapping_class() && storage && !Cmp(storage, "virtual") && !(nodetype && !Cmp(nodetype, "destructor"));
  }

  /* Key identifying one director virtual across classDirectorMethod (which buffers
   * the open func stub) and memberfunctionHandler (which emits the real open func
   * for methods wrapped in this class).  Name plus argument count is enough: the
   * two handlers derive it from the same node attributes, and overloads are matched
   * one-for-one because both run once per overload. */
  String *directorOpenKey(String *symname, ParmList *l) {
    String *nm = swiftMethodName(symname);
    String *key = NewStringf("%s/%d", Char(nm), emit_num_arguments(l));
    Delete(nm);
    return key;
  }
  ...
  virtual int classDirectorInit(Node *n) {
    ...
    director_open_stub_keys = NewList();
    director_open_stub_texts = NewList();
    director_open_emitted = NewHash();
  ...
  virtual int classDirectorMethod(Node *n, Node *parent, String *super) {
    ...
    for (int i = 0; i < num_args; i++) {
      ...
      SwigType *pt = Getattr(p, "type");
      String *arg_name = swiftParamLabel(p, i, seen_labels);
    ...
    Printf(director_cpp_methods, "    }\n");
    /* No callback installed: delegate to the base class (super_name is
     * "ClassName::methodName").  A pure virtual has no base body to call - emitting
     * one leaves the symbol undefined at link time - so raise instead, matching the
     * %ignore'd pure virtual above. */
    String *dm_storage = Getattr(n, "storage");
    String *dm_value = Getattr(n, "value");
    if (dm_storage && !Cmp(dm_storage, "virtual") && dm_value && !Cmp(dm_value, "0")) {
      String *pname = SwigType_namestr(Getattr(parent, "name"));
      Printf(director_cpp_methods, "    Swig::DirectorPureVirtualException::raise(\"%s::%s\");\n", pname, cpp_name);
      Delete(pname);
    } else if (is_void_ret)
    ...
    /* Buffered, not emitted: a virtual wrapped for this class gets a real open func
     * from memberfunctionHandler instead, and classHandler drops the matching stub.
     * What is left over are the inherited virtuals, which keep the stub. */
    String *stub = NewString("");
    if (is_void_ret) {
      Printf(stub, "    %sopen func %s(%s) throws {}\n", of_kw, open_fn, open_params);
    } else if (Len(sw_def) > 0) {
      Printf(stub, "    %sopen func %s(%s) throws -> %s { %s }\n", of_kw, open_fn, open_params, swift_ret, sw_def);
    } else {
      Printf(stub, "    %sopen func %s(%s) throws -> %s {}\n", of_kw, open_fn, open_params, swift_ret);
    }
    String *stub_key = directorOpenKey(c_symname, l);
    Append(director_open_stub_keys, stub_key);
    Append(director_open_stub_texts, stub);
    Delete(stub_key);
    Delete(stub);
    Delete(sw_def);
  ...
  virtual int classHandler(Node *n) {
    String *cn = Getattr(n, "sym:name");
    /* Swift-side spelling of the class name: a class named after a Swift keyword or
     * declaration modifier (e.g. "override") must be backticked where it is declared,
     * matching the escaped references produced by resolveSwiftClassName(). */
    String *swift_cn = swiftKeywordEscape(cn);
    ...
    if (needs_open) {
      if (baseclass_name)
        Printf(proxy_class_def, "\nopen class %s: %s {\n", swift_cn, baseclass_name);
      else
        Printf(proxy_class_def, "\nopen class %s {\n", swift_cn);
    } else if (baseclass_name) {
      Printf(proxy_class_def, "\n%s class %s: %s {\n", class_mods, swift_cn, baseclass_name);
    } else {
      Printf(proxy_class_def, "\n%s class %s {\n", class_mods, swift_cn);
    }
    ...
    Replaceall(swiftbody, "$swiftclassname", swift_cn);
    Printv(proxy_class_code, swiftbody, NIL);
    Delete(swiftbody);

    /* True only on the object a director was constructed from.  The open funcs use
     * it to tell "the subclass did not override this" from "this proxy wraps a C++
     * object and must dispatch virtually".  Declared once per director hierarchy. */
    if (director_flag && proxy_class_name) {
      if (!(current_class_has_base && ancestorHasGeneratedMethodWith(n, "swigDirectorSelf")))
        Printf(proxy_class_code, "    fileprivate var swigDirectorSelf: Bool = false\n");
      recordGeneratedMethod(proxy_class_name, "swigDirectorSelf");
    }
    ...
    if (director_flag && director_swift_init_cb) {
      if (Len(director_swift_inits) > 0) {
        ...
        Printf(proxy_class_code, "        swigDirectorSelf = true\n");
      ...
      /* Stubs for the virtuals memberfunctionHandler did not emit (inherited or
       * %ignore'd ones); the rest already have a real open func in the class body. */
      for (int si = 0; si < Len(director_open_stub_keys); si++) {
        String *stub_key = Getitem(director_open_stub_keys, si);
        String *emitted = Getattr(director_open_emitted, stub_key);
        int left = emitted ? atoi(Char(emitted)) : 0;
        if (left > 0) {
          String *dec = NewStringf("%d", left - 1);
          Setattr(director_open_emitted, stub_key, dec);
          Delete(dec);
          continue;
        }
        Printv(proxy_class_code, Getitem(director_open_stub_texts, si), NIL);
      }
    }
    ...
    /* ----- deinit / destructor -------------------------------------------- */
    {
      ...
      Replaceall(destruct_code, "$swiftclassname", swift_cn);
     ...
     * declare a stored property that swiftconstruct assigns.  No default. */
    {
      ...
      if (Len(swiftcode) > 0) {
        Replaceall(swiftcode, "$swiftclassname", swift_cn);
    ...
    if (vector_element_swifttype) {
      Printv(swift_module_code, "\nextension ", swift_cn, ": RandomAccessCollection {\n", NIL);
    ...
    if (director_flag) {
      ...
      director_swift_dispatch_fns = NULL;
      Delete(director_open_stub_keys);
      Delete(director_open_stub_texts);
      Delete(director_open_emitted);
      director_open_stub_keys = NULL;
      director_open_stub_texts = NULL;
      director_open_emitted = NULL;
  ...
  virtual int memberfunctionHandler(Node *n) {
    ...
    /* A virtual method of a director class is emitted below as an open func with a
     * real body (emitSwiftProxyMethod turns the access modifier into "open"), and
     * classHandler then drops the matching stub buffered by classDirectorMethod.
     * Skip virtual methods in non-director classes that derive from a director class
     * when no C wrapper was generated for this class specifically (the method's
     * wrap:name starts with _wrap_ but not with _wrap_<ClassName>_). */
    bool director_virtual = false;
    if (Getattr(n, "storage") && !Cmp(Getattr(n, "storage"), "virtual")) {
      director_virtual = director_flag;
      String *wn = Getattr(n, "wrap:name");
      if (!director_virtual && wn && proxy_class_name) {
    ...
    if (director_virtual && director_open_emitted) {
      String *key = directorOpenKey(Getattr(n, "sym:name"), Getattr(n, "parms"));
      String *seen = Getattr(director_open_emitted, key);
      String *inc = NewStringf("%d", (seen ? atoi(Char(seen)) : 0) + 1);
      Setattr(director_open_emitted, key, inc);
      Delete(inc);
      Delete(key);
    }
  ...
  void emitDirectorConstructor(Node *n, bool has_base) {
    ...
    Printf(director_swift_inits, "        swigDirectorSelf = true\n");
