...
// Element types passed as const references: std::vector's push_back and set take
// const value_type&, so every primitive element type needs a const-reference
// typemap or the element degrades to an opaque SWIGTYPE_p_* wrapper.
%template(ByteVector) std::vector<unsigned char>;
%template(SizeVector) std::vector<size_t>;
%template(ShortVector) std::vector<short>;
%inline %{
unsigned char byte_inc(const unsigned char &b) { return b + 1; }
size_t size_twice(const size_t &n) { return n * 2; }
%}
